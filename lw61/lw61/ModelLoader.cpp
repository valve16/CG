	...
	std::string const& baseFolder)
{
	...
	// Добавляем загрузку второй текстуры
	{
		Lib3dsTextureMap const& tex2 = materialInfo.texture2_map;
		if (*tex2.name) {
			try {
				CTexture2DHandle texture2 = LoadTexture(tex2.name, model, baseFolder);
				CTextureMap& textureMap2 = material.AddTextureMap2(texture2);
				InitTextureMap(tex2, textureMap2);
			}
			catch (std::runtime_error const&) {
				if (!m_ignoreMissingTextures) throw;
			}
		}
	}
}
