	...
	: m_pFrameBuffer(std::make_unique<CFrameBuffer>(1200, 600))
{
	...
	*/
	...
	//	cubeTransform.Translate(0.0, 0.0, 0.0);
	//	cubeTransform.Scale(1, 1, 1);
	//	//sphereTransform.Scale(1, 0.4, 1);
	//	auto cube = std::make_shared<CCube>(1);
	...
	//	auto cubeShader = std::make_shared<CSimpleDiffuseShader>();
	//	cubeShader->SetMaterial(material2);
	//	m_scene.AddObject(std::make_shared<CSceneObject>(std::move(cube), std::move(cubeShader)));
	//}

	// Создаем и добавляем в сцену точечный источник света
	{
		auto light = std::make_shared<COmniLightSource>(CVector3d(0.0, 0.0, 2.0));
