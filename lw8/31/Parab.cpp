...
bool CParaboloid::Hit(CRay const& ray, CIntersection& intersection) const
{
    ...
    // Проверка пересечения с крышкой (z = 1)

    double t = (1 - invRay.GetStart().z) / invRay.GetDirection().z;

    if (t > HIT_TIME_EPSILON)  
    {
        CVector3d point = invRay.GetPointAtTime(t);
        CVector3d worldPoint = ray.GetPointAtTime(t);

        if (point.x * point.x + point.y * point.y <= 1)
        {
            CVector3d normal(0, 0, 1);  
            CVector3d hitNormal0 = GetNormalMatrix() * normal;
    ...
    if (disc >= 0)
    {
        ...
        auto addHit = [&ray, &invRay, this, &intersection](double t) {
            if (t > HIT_TIME_EPSILON)
            {
                ...
                if (hitPoint0InObjectSpace.z >= 0 && hitPoint0InObjectSpace.z < 1)
                {
                    // Нормаль к параболоиду в точке задается градиентом функции (2x 2y -1)
                    CVector3d vec3(2 * hitPoint0InObjectSpace.x, 2 * hitPoint0InObjectSpace.y, -1);

                    CVector3d const& hitNormal0InObjectSpace = vec3;
